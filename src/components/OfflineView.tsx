...
import { WifiOff, AlertCircle, RefreshCw, Key, ShieldAlert, Wifi } from 'lucide-react';

export const OfflineView: React.FC = () => {
  const { isOnline, toggleOnline, orders } = useMealDirect();
    ...
    <AppShell activeTab="none">
      <div className="max-w-md mx-auto py-12" id="offline_recovery_page">
        <GlassPanel className="p-8 text-center border-t-4 border-t-amber-500 relative overflow-hidden">
          ...
          {/* Simulated connection network debug trigger */}
          <div className="mt-6 bg-amber-50/50 p-4 rounded-2xl border border-amber-200/60 text-xs text-left">
            <div className="flex items-center justify-between mb-3">
              <span className="font-bold text-amber-800">Connection Simulation HUD:</span>
              <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${isOnline ? 'bg-emerald-deep text-white' : 'bg-red-500 text-white'}`}>
                {isOnline ? 'Online mode' : 'Offline mode'}
              </span>
            </div>
            
            <p className="text-[10px] text-muted-grey mb-3">
              This interactive test trigger allows you to toggle internet latency to dry-run transaction failure recoveries.
            </p>

            <button
              onClick={toggleOnline}
              className="w-full py-2.5 bg-white hover:bg-neutral-50 border border-amber-300 text-amber-800 font-bold text-[10px] rounded-xl transition flex items-center justify-center gap-1.5 cursor-pointer"
              id="offline_simulation_toggle"
            >
              {isOnline ? (
                <>
                  <WifiOff className="w-3.5 h-3.5 text-danger" />
                  <span>Disconnect Simulated latency</span>
                </>
              ) : (
                <>
                  <Wifi className="w-3.5 h-3.5 text-emerald-strong" />
                  <span>Restore Simulated Connection</span>
                </>
              )}
            </button>
          </div>

...
export const OrderDetailView: React.FC<OrderDetailProps> = ({ orderId }) => {
  ...
  const handleSimulateDispatchStep = () => {
  ...
  return (
    <AppShell activeTab="orders">
      ...
      {['CONFIRMED', 'CANCELLED', 'REFUNDED', 'ESCALATED'].indexOf(order.status) === -1 && (
        <section className="mb-6" id="demo_simulation_box">
          <div className="bg-emerald-deep/5 border border-emerald-deep/12 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-xs font-bold text-emerald-strong">Review Order status Simulation</h4>
                <p className="text-[9px] text-muted-grey mt-0.5">Click through the courier workflow steps to track status alterations.</p>
              </div>
            </div>

            <button
              onClick={handleSimulateDispatchStep}
              className="px-4 py-2 bg-emerald-strong hover:bg-emerald-deep text-white font-bold text-[10px] rounded-xl flex items-center gap-1 cursor-pointer transition active:scale-95 shadow-xs"
              id="simulate_progress_step"
            >
              <Play className="w-3 h-3 text-mango-warm fill-mango-warm" />
              <span>Simulate Next Dispatch Step</span>
            </button>
          </div>
        </section>
      )}

...
import { formatNGN } from '../mockData';
...
export const GlassPanel: React.FC<{
  ...
  return (
    <div
      id={id}
      onClick={onClick}
      className={`glass-panel rounded-2xl p-6 transition-all duration-300 ${onClick ? 'cursor-pointer hover:shadow-md hover:scale-[1.01]' : ''} ${className}`}
    >
      {children}
    </div>
  );
...
export const OfflineBanner: React.FC = () => {
  const { isOnline, lastSyncTime, setOnlineStatus } = useMealDirect();
      ...
      {!isOnline ? (
        <div className="bg-red-950/90 border-b border-red-800 text-red-200 px-4 py-2.5 text-xs font-medium flex items-center justify-between shadow-inner">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>Offline Mode Active:</strong> Operating from local cache. Order placement is restricted.
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-[10px] text-red-300">Sync: {lastSyncTime}</span>
            <button
              onClick={() => setOnlineStatus(true)}
              className="bg-red-800 hover:bg-red-700 text-white rounded px-2.5 py-0.5 font-semibold transition active:scale-95 cursor-pointer"
            >
              Reconnect
            </button>
          </div>
        ...
        <div className="bg-emerald-950 text-emerald-200 px-4 py-1 text-[11px] font-medium flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
            <span>Secure connection established with Venite Campus central server.</span>
          </div>
          <button
            onClick={() => setOnlineStatus(false)}
            className="text-[10px] text-emerald-400 hover:underline cursor-pointer"
            title="Simulate network disconnection"
          >
            Go Offline
          </button>
        </div>
...
export const AppShell: React.FC<AppShellProps> = ({ children, activeTab }) => {
  const { user, navigateTo, signOut, notifications, isOnline, setOnlineStatus, cart, activeInAppAlert, dismissInAppAlert } = useMealDirect();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-canvas-ivory text-ink-deep font-sans">
      ...
      <header className="bg-white/40 backdrop-blur-xl border-b border-ink-deep/5 sticky top-0 z-40 h-20 px-4 md:px-8 flex items-center justify-between" id="global_header">
        <div className="flex items-center gap-3">
          <div
            onClick={() => navigateTo(user ? '/home' : '/')}
            className="flex items-center gap-2.5 cursor-pointer select-none animate-fade-in"
          >
            <div className="w-10 h-10 bg-emerald-deep rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-deep/20 transition duration-300 hover:scale-105">
            ...
            {navItems.map(item => {
              ...
              return (
                <button
                  ...
                  className={`relative p-3 rounded-2xl transition-all duration-200 group cursor-pointer ${
                    isActive
      ...
      {user && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-panel border-t border-emerald-deep/8 bg-white/95 px-2 py-2 flex justify-around items-center z-40 shadow-xl" id="mobile_bottom_nav">
          {navItems.map(item => {
            const Icon = item.icon;
            const isActive = activeTab === item.key;
            return (
              <button
                key={item.key}
                onClick={() => navigateTo(item.path)}
                className={`relative flex flex-col items-center justify-center p-2 rounded-2xl min-w-[56px] cursor-pointer transition-all ${
                  isActive ? 'text-emerald-deep font-bold scale-105' : 'text-muted-grey active:scale-95'
                }`}
                aria-label={item.label}
              >
